...
COMMAND_ALIASES = {
    "birds": "birds", ":birds": "birds", "history": "history", ":history": "history",
    "help": "help", ":help": "help", "?": "help", "status": "status", ":status": "status",
    "schema": "schema", ":schema": "schema", "quit": "quit", ":quit": "quit", ":q": "quit", "exit": "quit",
}
...
def build_engine(path: str | Path):
    ...
    registry.retain(ledger.enabled_plugin_ids())
...
def print_report(report) -> None:
    ...
    for opinion in report.opinions:
        ...
        for action in opinion.recommendations: print(f"  → {action}")
        for risk in opinion.risks: print(f"  ⚠ {risk}")
    print("\nBROTHER APE RULING\n" + report.decision.synthesis + "\n\nACTIONS")
    for action in report.decision.actions: print(f"  🍌 {action}")
    if report.decision.risks:
        print("\nRISKS")
        for risk in report.decision.risks: print(f"  ⚠ {risk}")
...
def print_replay(replay: dict) -> None:
    ...
    print(f"Topic: {replay['topic']['text']}\nIntegrity: {'PASS' if replay['integrity']['valid'] else 'FAIL'}")
    for opinion in replay["opinions"]: print(f"\n[{opinion['bird_id']}] {opinion['summary']}")
...
def parse_repl_command(raw: str) -> tuple[str | None, list[str]]:
    ...
    head = parts[0].lower()
    if head in {"replay", ":replay"}: return "replay", parts[1:]
    if head in {"plugins", ":plugins"}: return "plugins", parts[1:]
    return None, []
...
def print_help() -> None:
    print(
        "Commands:\n"
        "  birds | :birds                    List active birds\n"
        "  plugins list                      List discovered plugins and quarantine state\n"
        "  plugins enable <bird-id>          Enable a plugin for the next launch\n"
        "  plugins disable <bird-id>         Disable a plugin for the next launch\n"
...
def execute_repl_command(engine: AviaryEngine, command: str, args: list[str]) -> bool:
    ...
    if command == "help": print_help(); return True
    if command == "birds": print("\n".join(engine.registry.ids())); return True
    if command == "plugins":
        if not args or args == ["list"]:
            for item in engine.ledger.list_plugins():
                state = "enabled" if item["enabled"] else "disabled"
                print(f"{item['plugin_id']}\t{state}\t{item['module']}")
            return True
        if len(args) == 2 and args[0] in {"enable", "disable"}:
            enabled = args[0] == "enable"
            try:
                engine.ledger.set_plugin_enabled(args[1], enabled)
                print(f"Plugin {args[1]} {'enabled' if enabled else 'disabled'}; restart Aviary to apply.")
            except LookupError as exc:
                print(f"ERROR: {exc}")
            return True
        print("Usage: plugins list | plugins enable <bird-id> | plugins disable <bird-id>")
        return True
    if command == "history":
        rows = engine.ledger.recent_sessions()
        if not rows: print("No sessions recorded.")
        for row in rows: print(f"#{row['id']} {row['status']} {row['text']} {row.get('sha256') or ''}".rstrip())
        return True
    if command == "replay":
        if len(args) != 1 or not args[0].isdigit(): print("Usage: replay <session-id>"); return True
        try: print_replay(engine.ledger.replay_session(int(args[0])))
        except (LookupError, ValueError) as exc: print(f"ERROR: {exc}")
        return True
    if command == "status":
        ...
        return True
    if command == "schema": print(f"Ledger schema version: {engine.ledger.get_schema_version()}"); return True
    return True
...
def repl(engine: AviaryEngine) -> int:
    ...
    while True:
        ...
        if command:
            if not execute_repl_command(engine, command, args): return 0
            continue
...
def main(argv=None) -> int:
    ...
    try:
        if args.list_birds:
            for bird in engine.registry.all(): print(f"{bird.bird_id}\t{bird.instance.metadata().name}\t{bird.module}")
            return 0
        if args.replay is not None:
            replay = ledger.replay_session(args.replay)
            print(json.dumps(replay, indent=2) if args.json else "") if args.json else print_replay(replay)
            return 0
        if args.topic:
            report = engine.run(" ".join(args.topic))
            print(json.dumps(report.as_dict(), indent=2)) if args.json else print_report(report)
            return 0
        return repl(engine)
    finally:
        ledger.close()
